...
def get_hs_regions(
    ...
    max_frame: int,
) -> Tuple[List[Iterable[int]], List[Iterable[int]], List[Iterable[int]]]:
    """
...
class HoldShift:
    ...
    ) -> Dict[str, List[List[Iterable[int]]]]:
        assert (
            vad.ndim == 3
...
class Backchannel:
    ...
    def sample_negative_segment(self, region: Iterable[int]) -> Iterable[int]:
...
class TurnTakingEvents:
    ...
    @torch.no_grad()
    def __call__(self, vad: Tensor, max_time: Optional[float] = None) -> EVENTS:
        assert (
            vad.ndim == 3
